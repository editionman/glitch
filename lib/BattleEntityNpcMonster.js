...
monsterBattleEntity = cls.Class.extend({
  init: function(data) {
    ...
    this.ps=Math.round(10+(this.level/100*((base_ps*2)+iv_ps+(ev_ps/4)))+this.level);
    this.atk=Math.round(5+(this.level/100*((base_atk*2)+iv_atk+(ev_atk/4)))*(1+0));//al final multiplica por la potencia de la naturaleza que es de neutral=1---favor=1.25----contra=0.75
    this.def=Math.round(5+(this.level/100*((base_def*2)+iv_def+(ev_def/4)))*(1+0));
    this.atk_es=Math.round(5+(this.level/100*((base_atk_es*2)+iv_atk_es+(ev_atk_es/4)))*(1+0));
    this.def_es=Math.round(5+(this.level/100*((base_def_es*2)+iv_def_es+(ev_def_es/4)))*(1+0));
    this.velocidad=Math.round(5+(this.level/100*((base_velocidad*2)+iv_velocidad+(ev_velocidad/4)))*(1+0));
    
    this.evaluateMovs=movesObj(libMonsters.monster[data.monster_num].movimientos,this.level);//allmovesOBJ,lvlINT,movesaprendidosOBJ
    this.mov_1=this.evaluateMovs[1];
    this.mov_2=this.evaluateMovs[2];
    this.mov_3=this.evaluateMovs[3];
    this.mov_4=this.evaluateMovs[4];
...
function movesObj(allmoves,level,availableMoves){
  var result={}
  result={
