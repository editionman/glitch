...
monsterEntity = cls.Class.extend({
  init: function(monsternum,userid) {
    ...
    this.evaluateMovs=moveObjForDB(libMonsters.monster[monsternum].movimientos,this.nivel,{});//allmovesOBJ,lvlINT,movesaprendidosOBJ
    ...
    this.ps=Math.round(10+(this.nivel/100*((this.base_ps*2)+this.iv_ps+(this.ev_ps/4)))+this.nivel);
    this.atk=Math.round(5+(this.nivel/100*((this.base_atk*2)+this.iv_atk+(this.ev_atk/4)))*(1+0));//al final multiplica por la potencia de la naturaleza que es de neutral=1---favor=1.25----contra=0.75
    this.def=Math.round(5+(this.nivel/100*((this.base_def*2)+this.iv_def+(this.ev_def/4)))*(1+0));
    this.atk_es=Math.round(5+(this.nivel/100*((this.base_atk_es*2)+this.iv_atk_es+(this.ev_atk_es/4)))*(1+0));
    this.def_es=Math.round(5+(this.nivel/100*((this.base_def_es*2)+this.iv_def_es+(this.ev_def_es/4)))*(1+0));
    this.velocidad=Math.round(5+(this.nivel/100*((this.base_velocidad*2)+this.iv_velocidad+(this.ev_velocidad/4)))*(1+0));
  },
...
function moveObjForDB(allmoves,level,availableMoves){
  ...
  result.battlemoves={
    1:{id:null},//{id:null,coldown:null},//{id:null,pp:null,maxpp:null}
    2:{id:null},//{id:null,coldown:null},//{id:null,pp:null,maxpp:null}
    3:{id:null},//{id:null,coldown:null},//{id:null,pp:null,maxpp:null}
    4:{id:null},//{id:null,coldown:null},//{id:null,pp:null,maxpp:null}
  };
  for(var i=0;i<allmoves.length;i++){
    var idMove=allmoves[i][0];
    allmoves[i][4]=false;
    //allmoves[i][0]--->idmove
    //allmoves[i][1]--->name
    ...
    if(level>=allmoves[i][2] && result.allmoves[allmoves[i][0]]===allmoves[i][1] && result.battlemoves[1].id===null && allmoves[i][4]===false){//aprender en el primero
      result.battlemoves[1]={id:allmoves[i][0]};//{id:allmoves[i][0],coldown:libMoves[idMove].coldown};//{id:allmoves[i][0],pp:10,maxpp:10};
      allmoves[i][4]=true;
    }
    if(level>=allmoves[i][2] && result.allmoves[allmoves[i][0]]===allmoves[i][1] && result.battlemoves[2].id===null && allmoves[i][4]===false){//aprender en el segundo
      result.battlemoves[2]={id:allmoves[i][0]};//{id:allmoves[i][0],coldown:libMoves[idMove].coldown};//{id:allmoves[i][0],pp:10,maxpp:10};
      allmoves[i][4]=true;
    }
    if(level>=allmoves[i][2] && result.allmoves[allmoves[i][0]]===allmoves[i][1] && result.battlemoves[3].id===null && allmoves[i][4]===false){//aprender en el tercero
      result.battlemoves[3]={id:allmoves[i][0]};//{id:allmoves[i][0],coldown:libMoves[idMove].coldown};//{id:allmoves[i][0],pp:10,maxpp:10};
      allmoves[i][4]=true;
    }
    if(level>=allmoves[i][2] && result.allmoves[allmoves[i][0]]===allmoves[i][1] && result.battlemoves[4].id===null && allmoves[i][4]===false){//aprender en el cuarto
      result.battlemoves[4]={id:allmoves[i][0]};//{id:allmoves[i][0],coldown:libMoves[idMove].coldown};//{id:allmoves[i][0],pp:10,maxpp:10};
      allmoves[i][4]=true;
    }
  }
  return result;
