...
monsterPlayerEntity = cls.Class.extend({
  init: function(data,partyName,lastTime) {
    ...
    this.currPS=this.ps;//este debe venir de base de datos cuando se agrege--->indica cuanta vida tiene o si esta muerto
    this.state="none";  //este debe venir de base de datos cuando se agrege--->indica si esta envenenado congelado quemado,etc
  ...
  takeDMG:function(dmgQty){
    var dmgRecived=0;
    var isLive=true;
    if(this.currPS-dmgQty<=0){
      dmgRecived=this.currPS;
      isLive=false;
      this.currPS=0;
      ...
      dmgRecived=dmgQty;
      isLive=true;
      this.currPS=this.currPS-dmgQty;
    }
    return {dmgRealized:dmgRecived,isLive:isLive};
  },
  ...
  takeExp:function(qty){
    if(this.exp+qty>=this.maxExp){
      var newExp=qty-(this.maxExp-this.exp);
      this.nivel=this.nivel+1;
      this.maxExp=this.nivel*this.nivel*this.nivel;
      this.exp=0;
      if(newExp>0)this.takeExp(newExp);
    }else this.exp=this.exp+qty;
  },
  ...
  //-------------GUARDAR INFO CUANDO EL POKEMON MUERE O ES CAMBIADO
  saveDATA:async function(conexion){
    var userID=this.owner;
    var monID=this.monster_id;
    var monInfo=this;
    const result=await saveDB.SaveMonsterBatle(conexion,monInfo);
    if(result.object===null){
      console.log("No se pudo guardar info de este pokemon en DB: "+monID+" User: "+userID);
    }
  },
});
