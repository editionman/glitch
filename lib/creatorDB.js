...
creatorsql.CreateStarterTrainer= function(conexion,personajeNum,data,plr){
  ...
  var sql1="";
  var sql2="";
  console.log(data.mov_1);
  //console.log(plr.team[slot]);
  //return new Promise((resolve,reject)=>{});
  return new Promise((resolve,reject)=>{
    if(slot!==null){
      //allMoves
      sql1="INSERT INTO monsters(monster_id,monster_num,monster_name,special,user_current_owner,user_owner,in_team,genero,type_1,type_2,exp,naturaleza,habilidad,mov_1,mov_2,mov_3,mov_4,allmoves,iv_ps,iv_atk,iv_def,iv_atk_es,iv_def_es,iv_velocidad) VALUES (NULL,'"+data.monster_num+"','"+data.monstername+"',0,'"+plr.user_id+"','"+plr.user_id+"',"+slot+","+data.genero+","+data.type_1+","+data.type_2+",0,"+data.naturaleza+","+data.habilidad+",'"+JSON.stringify(data.mov_1)+"','"+JSON.stringify(data.mov_2)+"','"+JSON.stringify(data.mov_3)+"','"+JSON.stringify(data.mov_4)+"','"+JSON.stringify(data.allMoves)+"',"+data.iv_ps+","+data.iv_atk+","+data.iv_def+","+data.iv_atk_es+","+data.iv_def_es+","+data.iv_velocidad+");";
      //sql1+="UPDATE users SET personaje='"+plr.personaje+"', team='"+plr.team+"' WHERE user_id='"+plr.user_id+"'";
    }
    else{
      sql1="INSERT INTO monsters(monster_id,monster_num,monster_name,special,user_current_owner,user_owner,in_team,genero,type_1,type_2,exp,naturaleza,habilidad,mov_1,mov_2,mov_3,mov_4,allmoves,iv_ps,iv_atk,iv_def,iv_atk_es,iv_def_es,iv_velocidad) VALUES (NULL,'"+data.monster_num+"','"+data.monstername+"',0,'"+plr.user_id+"','"+plr.user_id+"',"+0+","+data.genero+","+data.type_1+","+data.type_2+",0,"+data.naturaleza+","+data.habilidad+",'"+JSON.stringify(data.mov_1)+"','"+JSON.stringify(data.mov_2)+"','"+JSON.stringify(data.mov_3)+"','"+JSON.stringify(data.mov_4)+"','"+JSON.stringify(data.allMoves)+"',"+data.iv_ps+","+data.iv_atk+","+data.iv_def+","+data.iv_atk_es+","+data.iv_def_es+","+data.iv_velocidad+");";
