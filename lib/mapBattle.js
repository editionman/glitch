...
battleMap=cls.Class.extend({
  init:function(id,mapName){
    ...
    this.team1 = {};//objects de hasta 3 participantes
    this.team2 = {};//objects de hasta 3 participantes
    //this.monsters = {};
    //this.players = {};
    this.viewers = {};
    this.arbitros = {};
  ...
  AddTeam: function(teamnum,player){//A o B
    var self=this;
    this["team"+teamnum][player.user_id]={user_id:0,user_char:1,team:{},itembattle:{}}//aca va la info de los jugadores
  ...
  NPCDialog: function(conexion,websocket,npcID,npcGUID){
    ...
    this.npcs[npcID].questActive(conexion,websocket.userID,this.npcs[npcID]).then((numQuest)=>{
      ...
      if(numQuest>=gNPCS[this.npcs[npcID].idnpc].quests.length){//Ya completo todas las quest de este npc
        ...
        console.log("hasta aca me quede en battleMap, ahora que el usuario reciba la info del quest");
      }
    });
  },
  //##############################################################################################
