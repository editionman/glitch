...
worldMap=cls.Class.extend({
  ...
  moveContact:function(data,websocket){//id,rivalOwner,rivalMonster,type
    ...
    if(resultMove.dmgRival>0){
      ...
      if(data.type==="wild"){
        websocket.broadcast.to(player.mapCode).emit('battleDmg',{monid:rival.monster_id,owner:rival.spawn,dmg:resultMove.dmgRival,type:data.type});
        websocket.emit("battleDmg",{monid:rival.monster_id,owner:rival.spawn,type:data.type,exp:expResult});
      }
      else if(data.type==="trainer"){
        websocket.broadcast.to(player.mapCode).emit('battleDmg',{monid:rival.monster_id,owner:rival.owner,dmg:resultMove.dmgRival,type:data.type});
        websocket.emit("battleDmg",{monid:rival.monster_id,owner:rival.spawn,type:data.type,exp:expResult});
      }
