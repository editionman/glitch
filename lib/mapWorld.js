...
var battleActions = require("../data/battleActions.js");
//-----------------------------------
...
worldMap=cls.Class.extend({
  ...
  createAtkMonster:function(websocket,data){
    ...
    if(monMove===undefined)return;//no esta creado el mov en base de datos asi que esto previene el crash
    if(libMove[monMove.id]===undefined)return;//no esta creado el mov en el servidor
  ...
  moveContact:function(data,websocket){//id,rivalOwner,rivalMonster,type
    ...
    if(resultMove.dmgRival>0){
      var expResult=1;
      var rivalAtk=rival.takeDMG(resultMove.dmgRival);//dmgRealized,isLive:::boolean/////resultMove.dmgRival
      mymon.takeExp(Math.ceil(rivalAtk.dmgRealized/mymon.nivel));//###dar 1 de experiencia por mientras
...
function executeMove(mon,rival,move){
  ...
  //-------------------------------------
  var bonificacion=(mon.type1==move.type || mon.type2==move.type)?1.5:1;
  var efectividad=battleActions.efectividadDMG(move.type,rival.type1)*battleActions.efectividadDMG(move.type,rival.type2);
  var inestable=utils.ranInt(85,100);
  ...
  var dam1=0.01*bonificacion*efectividad*inestable;
  var dam2=Math.round((((0.2*level+1)*ataque*poder)/(25*defensa))+2);
  result.dmgRival=dam1*dam2;
  return result;
}
