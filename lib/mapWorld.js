...
worldMap=cls.Class.extend({
  ...
  createMonsterBattle:async function(websocket,conexion,data){
    ...
    if(result.info==="SUCCESS")newMonsterOnBattle=new playerMonsterEntity(result.object[0]);
    if(newMonsterOnBattle!==null && this.trainerMonsters[player.userID]===undefined){//no se puede sacar el pokemon de nuevo por que detecta que  ya existe con la primera vez
      ...
      console.log("EN SERVER ME QUEDE ACA mapWorld");
    }
    //verificar que el monster sea de esta persona y se encuentre en team
