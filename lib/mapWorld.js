...
worldMap=cls.Class.extend({
  ...
  monsterMapDefeat: function(websocket){
  ...
  monsterStartBattle: function(monsterID,websocket){
    var data={num:this.players[websocket.userID].wildMon.monid,special:this.players[websocket.userID].wildMon.especial,name:this.players[websocket.userID].wildMon.name,level:this.players[websocket.userID].wildMon.level,ambiente:this.players[websocket.userID].wildMon.ambiente};
    ...
    websocket.broadcast.to(websocket.mapCode).emit('OtherPlayerInWildBattle',{playerID:websocket.userID,inBattle:true,index:monsterID});
    *///####################################################################################################
    //Obtener los datos de los participantes de la batalla y el mapa
    //crear el mapa en js object donde se almacenaran todos los datos y desde donde se manejará el combate
    //trasladarlos al mapa de batalla en socket
