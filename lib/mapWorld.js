...
worldMap=cls.Class.extend({
  ...
  createAtkMonster:function(websocket,data){
    ...
    if(timePassed>=monMove.coldown*1000){
      ...
      this.moveLife(this.atkMonsters[player.userID][newAtk.id],libMove[monMove.id].life*1000,player.userID);
  ...
  //##############################################################################################
  moveLife:function(moveActive,lifeTime,userid){
    setTimeout(function(){
      console.log("se debe destruir este move por que acabo su tiempo de vida:"+moveActive.name);
    },lifeTime);//tiene un tiempo de vida y se detruye
  },
  moveDestroy:function(moveActive,userid){
    delete this.atkMonsters[userid][moveActive.id];
  ...
  moveContact:function(data,websocket){//id,rivalOwner,rivalMonster,type
    var player=websocket.player;
    console.log("el move hizo contacto con un monster rival");
    var rival=null;
    var mymon=null;
    var move=null;
    var resultMove={}
    ...
    if(resultMove.dmgRival>0){
      rival.takeDMG(resultMove.dmgRival);
      console.log(rival.currPS+"---"+rival.ps);
    }
    if(resultMove.dmgAlly>0){
      mymon.takeDMG(resultMove.dmgAlly);
    }
    //hacer los daños
