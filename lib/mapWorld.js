...
worldMap=cls.Class.extend({
  ...
  createMonsterBattle:async function(websocket,conexion,data){
    ...
    if(newMonsterOnBattle!==null){//no se puede sacar el pokemon de nuevo por que detecta que  ya existe con la primera vez
      if(this.trainerMonsters[player.userID]!==undefined){this.trainerMonsters[player.userID].saveDATA(conexion);delete this.trainerMonsters[player.userID];}
      ...
      websocket.broadcast.to(player.mapCode).emit('monsterPlayerJoined',{monster:newMonsterOnBattle,ball:"Pokeball"});
      websocket.emit("monsterPlayerJoined",{monster:newMonsterOnBattle,ball:"Pokeball"});//primero se envia la info al usuario para que invoque al monster y obtener la posicion
  ...
  createAtkMonster:function(websocket,data){
    ...
    if(this.trainerMonsters[player.userID]===undefined)return;
    ...
    if(libMove[monMove.id]===undefined)return;//no esta creado el mov en base de datos asi que esto previene el crash
    ...
    if(timePassed>=monMove.coldown*1000){
      ...
      websocket.emit("AtkJoined",newAtk);
    }    
  },
  //##############################################################################################
  //NPC Functions
  ...
  wildMonsterAction: function(data,websocket){
    ...
    else if(this.monsters[data.spawn][data.id]===undefined)return;
  ...
  monsterMapDefeat: function(wildmon,websocket){
    var player=websocket.player;
    var monsterID=wildmon.monster_id;
    delete this.monsters[wildmon.spawn][wildmon.monster_id];//-DELETE
    ...
    this.monsters[wildmon.spawn][newMonster.monster_id]=newMonster;
    websocket.broadcast.to(player.mapCode).emit('wildMonUpdateMap',{index:monsterID,spawn:wildmon.spawn,state:"die",newMon:newMonster});
    websocket.emit("wildMonUpdateMap",{index:monsterID,spawn:wildmon.spawn,state:"die",newMon:newMonster});
  },
  monsterTrainerDefeat:function(mon,websocket){
    var player=websocket.player;
    var trainerID=mon.owner.toString();
    delete this.trainerMonsters[mon.owner.toString()];
    
    websocket.broadcast.to(player.mapCode).emit('trainerMonUpdateMap',{index:trainerID,state:"die"});
    websocket.emit("trainerMonUpdateMap",{index:trainerID,state:"die"});
  },
  //##############################################################################################
  //Player Functions
  //##############################################################################################
  ...
  playerLeft: function(conexion,websocket){
    ...
    if(this.players[player.userID].host===true)this.haveHost=false;//cambiar host
    if(this.trainerMonsters[player.userID]!==undefined)this.trainerMonsters[player.userID].saveDATA(conexion);
  ...
  moveDestroy:function(moveActive,websocket){
    var player=websocket.player;
    if(moveActive===undefined)return;
  ...
  moveContact:function(data,websocket){//id,rivalOwner,rivalMonster,type
    ...
    if(resultMove.dmgRival>0){
      var rivalAtk=rival.takeDMG(1);//dmgRealized,isLive:::boolean/////resultMove.dmgRival
      ...
      if(rivalAtk.isLive===false && data.type==="trainer")this.monsterTrainerDefeat(rival,websocket);//destroy TRAINER monster-ESTE FALTA DEFINIR
      
      var expResult=1;
      mymon.takeExp(expResult);//###dar 1 de experiencia por mientras
      
      var objetiveData={hp:rival.currPS,state:rival.state};//###por mientras que mande toda la info-->aunque solo es necesario el hp,status,state
      websocket.broadcast.to(player.mapCode).emit('battleDmg',{atacado:rival,atacante:mymon,type:data.type});
      websocket.emit("battleDmg",{atacado:rival,atacante:mymon,type:data.type});
      /*
      if(data.type==="wild"){
        websocket.broadcast.to(player.mapCode).emit('battleDmg',{monid:rival.monster_id,owner:rival.spawn,objetive:rival,type:data.type});
        websocket.emit("battleDmg",{monid:rival.monster_id,owner:rival.spawn,objetive:rival,type:data.type,mymon:mymon});
      }
      else if(data.type==="trainer"){
        websocket.broadcast.to(player.mapCode).emit('battleDmg',{monid:rival.monster_id,owner:rival.owner,objetive:rival,type:data.type});
        websocket.emit("battleDmg",{monid:rival.monster_id,owner:rival.spawn,objetive:rival,type:data.type,mymon:mymon});
      }
      */
      //console.log(rival.currPS+"---"+rival.ps+" ACA HACER QUE LA VIDA QUITADA SE CONVIERTA EN EXP PARA EL POKE");
      
      //POR MIENTRAS DAR UNO DE EXP AL ATACANTE Y ENVIAR EL NUEVO STATUS DE AMBOS A TODO EL MUNDO
      //AL DUEÑO DEL ATAQUE ENVIARLE LA EXP GANADA Y OTRA INFO EXTRA SI LE OCASIONÓ
    }
...
function crearMonsterWild(name,monInfo,monsternum,arrLevel,spawn,ambiente){
  ...
  var newMon=new wildMonsterWorldEntity(monsternum,lvlMonsterMap(arrLevel),spawn,ambiente);
  return newMon;
}
...
function executeMove(mon,rival,move){
  if(mon===undefined || rival===undefined || move===undefined)return;
  var result={};
  //result.rival={};
  //result.ally={};
