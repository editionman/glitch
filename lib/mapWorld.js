...
worldMap=cls.Class.extend({
  ...
  createAtkMonster:function(websocket,data){
    ...
    if(timePassed>=monMove.coldown*1000){
      ...
      this.atkMonsters[player.userID]=newAtk;
      websocket.broadcast.to(player.mapCode).emit('AtkJoined',newAtk);
      websocket.emit("atkInvoke",newAtk);//primero se envia la info al usuario para que invoque al monster y obtener la posicion
      //debe regresar al usuario y a todos los demas el skill creado con sus valores
    }
    console.log("En SKILL ME QUEDE ACA mapWorld.js");
