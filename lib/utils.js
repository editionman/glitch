...
Utils.StatPromedio= function(data,stat,baseStat){
    ...
    if(stat==="def_es"){
        base=baseStat;//stat base
        iv=data.iv_def_es;//16
        ev=data.ev_def_es;//0
        naturaleza=valueNaturaleza(stat,data.naturaleza);//0
      console.log(Math.round(5+(nivel/100*((base*2)+iv+(ev/4)))*(1+naturaleza)));
        return Math.round(5+(nivel/100*((base*2)+iv+(ev/4)))*(1+naturaleza));
    }
    if(stat==="velocidad"){
        base=baseStat;
