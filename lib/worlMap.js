...
var npcEntity = require("./npcEntity.js");
...
worldMap=cls.Class.extend({
  ...
  //NPC Functions
  //##############################################################################################
  NPCDialog: function(conexion,websocket,npcID){
    var self=this;
    //this.npcs[npcID].uid
    //this.npcs[npcID].idnpc
    this.npcs[npcID].questActive(conexion,websocket.userID,this.npcs[npcID]).then((numQuest)=>{
      //this.npcs[npcID].activeQuest=numQuest
      this.npcs[npcID].activeQuest=gNPCS[this.npcs[npcID].idnpc].quests[numQuest];//numero de quest
      //VERIFICAR SI SE COMPLETO EL QUEST SI ES ASI ENTONCES DAR RECOMPENSA Y FINALIZAR QUEST ACTUAL EN EL SERVIDOR Y LA DB
      var dataQuest={currQuest:gQuests[this.npcs[npcID].activeQuest],status:"incomplete"}
      //console.log("hasta aca me quede en worldMap, ahora que el usuario reciba la info del quest");
      websocket.emit("questUpdateMap",dataQuest);
  ...
  monsterSelectBattle: function(data,websocket){
    ...
    Object.keys(this.monsters).find(function(spawnid){
      if(self.monsters[spawnid][monsterID] !== undefined && self.monsters[spawnid][monsterID].rival===null && self.players[websocket.userID].wildMon==null){
        ...
        var dataUser={special:mon.especial,num:mon.monid,name:mon.name,level:mon.level,index:monsterID};
...
function crearMonsterWild(name,monInfo,numMon,arrLevel,spawn){
