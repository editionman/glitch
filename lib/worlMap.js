...
worldMap=cls.Class.extend({
  ...
  //NPC Functions
  //##############################################################################################
  NPCDialog: function(websocket,npcID){
    var self=this;
    console.log(this.npcs);
    console.log(this.npcs[npcID]);
    console.log(this.npcs[npcID].uid);
  },
  //##############################################################################################
  //Monster Functions
  //##############################################################################################
...
function foundMonster(array,arrLevel,spawn){
  ...
  for(var i=0;i<array.length;i++){
    ...
    if(isCreated==true){//se crea monster
      newMonsterFounded=crearMonsterWild(monsters.monster(array[i]).monstername,monsters.monster(array[i]),array[i],arrLevel,spawn);
...
function Npc(name,moveType,char){
  //this.id=0;
  this.uid=uuid.v4();
  this.name = name;
    this.char=char;
...
function spawnNPCSFunc(npcs){
  ...
  for(var id in npcs){
    ...
    var npc=npcs[id];
    var tempNPC=new npcEntity(gNPCS[npc[0]].name,gNPCS[npc[0]].moveType,gNPCS[npc[0]].char);
    //var tempNPC=new Npc(gNPCS[npc[0]].name,gNPCS[npc[0]].moveType,gNPCS[npc[0]].char);
